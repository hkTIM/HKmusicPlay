...
- (void)musicPlayerWithArray:(NSArray *)musicArray andIndex:(NSInteger )index{
    ...
    for (MusicListModel * obj in musicArray) {
        AVPlayerItem *songItem = [[AVPlayerItem alloc] initWithURL:[NSURL URLWithString:obj.url]];
        [self.musicArray addObject:songItem];
        [self.imageArray addObject:obj.albumpic_big];
        [self.songNameArray addObject:obj.songname];
        [self.singerArray addObject:obj.singername];
    }
    //播放歌单列表序号对应的歌曲
    [self.player replaceCurrentItemWithPlayerItem:self.musicArray[index]];
    //对当前item 添加观察者
